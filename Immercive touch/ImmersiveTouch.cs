...
namespace ImmersiveTouch
{
    public class ImmersiveTouch : MelonMod
    {
        ...
        public static CameraHaptic ConfigureCameraHaptic(InputDevice inputDevice, Transform hand, Transform CamAnglePoint, Transform CamSizePoint, float viewHeight)
        {
            if (inputDevice == null) return null;
            
            Transform cameraHapticObject = new GameObject().transform;
            cameraHapticObject.parent = CamAnglePoint;
            ...
            cameraHaptic.camera.orthographicSize = Vector3.Distance(hand.position, CamSizePoint.position) / ORTHOGRAPHIC_SIZE_MOD;
